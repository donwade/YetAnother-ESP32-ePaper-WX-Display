...
#if 0
static int (*fun_ptr)(const char *, ...) = &printf;
#else
static int ignore(const char *, ...) { return 0;}
int (*fun_ptr)(const char *, ...) = &ignore;
#endif

#define Debug(fmt,...) (*fun_ptr)(fmt,__VA_ARGS__)
...
void GxEPD2_EPD::_waitWhileBusy(const char* caller_fn, uint16_t line)
{
	...
	/*
	Debug("peak_lo = %3.2fv hi = %3.2fv adc > slice (%3.2fv > %3.2fv)\n",
	    BINARY2VOLT(peak_lo),
	    BINARY2VOLT(peak_hi),
	    BINARY2VOLT(adc),
	    BINARY2VOLT(slice)
